...
| `dataset/physician/` | **Primary physician study** (completes the 2×2), `build_physician_study.py`: `PHYSICIAN_ABSOLUTE_RUBRIC.xlsx` — Nature-format, **one blinded answer per row** (no competing answer), **158 response-items** (79 pairs × 2), 5 Real-POCQi axes 1–4 + competence/confidence; `PHYSICIAN_PAIRWISE.xlsx` — blinded A/B (randomized per item **and** reviewer), 5 A/B/tie + overall. Each has Instructions / Rubric-anchors / Worked-example / **Data-dictionary** / Ratings sheets, protected + drop-down-validated + provider-scrubbed. `packets/` (git-ignored) = 24 reviewer-specific files (15–25 items, ≥2 raters/item); `author_only/` (git-ignored) = un-blinding manifests + assignment + data dictionary |
