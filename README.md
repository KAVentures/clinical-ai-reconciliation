...
**Exact common-support decomposition (identical keys for A, B, C; 108 q×opp/axis).** The format component (C−B) is negative on all five axes (−17.8 to −43.1 pp, CIs exclude zero). The human→LLM rater component (B−A) is **null on accuracy (−7.9 pp [−23.5, +7.1], p=0.30)** and clinical utility, but significantly **positive** on source quality (+25.1), completeness (+17.6) and verifiability (+30.0) — LLM pairwise judges are *more* OE-favorable than physicians on the evidence axes. So we do **not** claim "instrument, not the rater" (the rater is a real, significant contributor), nor that the rater is large-and-negative on accuracy — an earlier revision's "−24.0 pp, significant" was an analysis error (unmatched cell A), now fixed. **Both matter, but the format carries the reversal.** Three-cell path decomposition, not a factorial (the {rubric, human} cell is missing); "~83% instrument share" withdrawn.
...
- **Both the rater and the format matter — we do *not* claim "instrument, not the rater," but the format carries the reversal.** On exact common support the rater component is null on accuracy (−7.9 pp, p=0.30) and significantly *positive* on the evidence axes (+18 to +30); it does not explain the rubric reversal. Three-cell path decomposition (missing the human-rubric cell), not a factorial.
