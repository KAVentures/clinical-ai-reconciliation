...
| `judge/blinding.py` | **Canonical blinded answer rendering** (`render_blinded_answer`) shared by the physician workbooks and the LLM regrade, so C and D see identical answer text |
...
| `dataset/physician/` | **Primary physician study** (fills the missing human-rubric cell D), `build_physician_study.py` (+ `test_physician_study.py`): **essential** `PHYSICIAN_ABSOLUTE_RUBRIC.xlsx` — Nature-format, **one blinded answer per row** (no competing answer), **160 response-items** (80 pairs × 2, un-deduped, one pair per distinct question), 5 axes 1–4 + competence/confidence; **optional** `PHYSICIAN_PAIRWISE.xlsx` (`--pairwise`). Disagreement-enriched **with tie/near-tie + agreement controls**. Each workbook: Instructions / Rubric-anchors / Worked-example / **Data-dictionary** / Ratings, protected, drop-down-validated, provider-scrubbed, all-fields completion counter, randomized order. **One scalable balanced-incomplete-block assignment** — `--doctors`/`--max-per-doctor`/`--target-ratings` set replication only (2 doctors→1 rating each; 3→+overlap); every response ≥1 rating, no doctor repeats a question, provider/opponent/specialty balanced. `packets/` + `author_only/` git-ignored. `analyze_returns.py` = essential D-scores model (provider effect, **doctor fixed effect**, question-clustered) + optional 2×2 OE-superiority estimand (win-score / probability-of-superiority / DiD, question-cluster bootstrap); `allocate_reviewers.py` = balanced random roster→doctor-ID mapping. See `HUMAN_STUDY_PROTOCOL.md` |
